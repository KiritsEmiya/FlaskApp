...
def movie_del(id=None):
    movie = Movie.query.get_or_404(int(id))  # 以后删除时还要删除电影评论及电影收藏
...
def movie_edit(id=None):
    ...
    if form.validate_on_submit():
        ...
        if form.url.data.filename != "":
            ...
            # print(form.url.data.filename)
            # print(file_url)
            movie.url = change_filename(file_url) + "." + str(form.url.data.filename).split('.')[1]
            # print(movie.url)
        ...
        if form.logo.data.filename != "":
            ...
            # print(form.logo.data.filename)
            # print(file_logo)
            movie.logo = change_filename(file_logo) + "." + str(form.logo.data.filename).split('.')[1]
            # print(movie.logo)
...
def auth_edit(id=None):
    ...
    if form.validate_on_submit():
        ...
        # 检查权限名称是否已经存在
        if auth_name == 1 and auth.name != data["name"]:
            flash("权限名称已存在！", "err")
            return redirect(url_for('admin.auth_edit', id=id))
        # 检查权限地址是否已经存在
        if auth_url == 1 and auth.url != data["url"]:
            flash("权限地址已存在！", "err")
            return redirect(url_for('admin.auth_edit', id=id))
        auth.url = data["url"]
        auth.name = data["name"]
